...
- The curve is exactly a circle if points are evenly spaced from a circle.
- A circle can be perfectly constructured by [repeating 3 points of a
  equilateral triangle][triangle]. (The same applies to [4 points on a
  square][square].)
...
Second, let (1) $q_i = p_i + t_i^L$ be the points on the angle bisectors where
$L$ operator rotates a vector 90 degrees left, and (2) all points $p_i, q_i$ are
represented by homogeneous coordinates. We then compute focals
